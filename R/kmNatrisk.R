...
#'
#' @param km output from \code{survfit} function in \code{survival} package.
#' @return draws Kaplan Meier curves with N at risk.
#' @author JG



kmNatrisk <- function(km, mark=3, simple=FALSE, xaxis.at=pretty(km$time), xaxis.lab=xaxis.at, lty.surv=1, lwd.surv=1, col.surv=1,
                      ...
                      label.n.at.risk=FALSE, draw.lines=TRUE, cex.axis=1, xlab='', ylab='', main='',
                      xlim=c(0,max(km$time)), ylim=c(0,1), grid=TRUE, lty.grid=1, lwd.grid=1, col.grid=grey(.9),
                      legend=!is.null(km$strata), loc.legend='topright', add=FALSE, ... # ... is passed to par()
){
