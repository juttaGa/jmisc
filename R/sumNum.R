...
sumNum <- function(data, grp=NULL, rd=2){
  ...
  if(is.null(grp)){
    sum <- num %>% summarise_each(funs(N=length(which(!is.na(.))), miss=length(which(is.na(.))), Minimum=min(., na.rm=T), Q1=quantile(., na.rm=T)[2],
                                       Median=median(.,na.rm=T), Q3=quantile(., na.rm=T)[4], Maximum=max(.,na.rm=T), Mean=mean(.,na.rm=T), Sd=sd(.,na.rm=T)))
    sum <- sum %>% gather(key, value) %>% separate(key, into = c("variable", "what"), sep = "\\_") %>% spread(what, value)
    sum <- sum %>% dplyr::select(variable, N, miss, Minimum, Q1, Median, Q3, Maximum, Mean, Sd)
  }
  else if(length(grp)==1){
    num <- num %>% mutate(group=data[,grp])
    sum <- num %>% group_by(group) %>% summarise_each(funs(N=length(which(!is.na(.))), miss=length(which(is.na(.))), Minimum=min(., na.rm=T), Q1=quantile(., na.rm=T)[2],
                                      Median=median(.,na.rm=T), Q3=quantile(., na.rm=T)[4], Maximum=max(.,na.rm=T), Mean=mean(.,na.rm=T), Sd=sd(.,na.rm=T)))
    sum <- sum %>% gather(key, value, -group) %>% separate(key, into = c("variable", "what"), sep = "\\_") %>% spread(what, value)
    sum <- sum %>% dplyr::select(variable, group, N, miss, Minimum, Q1, Median, Q3, Maximum, Mean, Sd)
  }
  else{
    num <- num %>% mutate(group1=data[,grp[1]], group2=data[,grp[2]])
    sum <- num %>% group_by(group1, group2) %>%
      summarise_each(funs(Minimum=min(., na.rm=T), Q1=quantile(., na.rm=T)[2], Median=median(.,na.rm=T), Q3=quantile(., na.rm=T)[4],
